...
int insert_symbol(char *name, unsigned kind, unsigned type, 
                  unsigned atr1, unsigned atr2, unsigned level){
...
unsigned lookup_level(char *name, unsigned kind) {
	unsigned i;
  for(i = first_empty - 1; i > FUN_REG; i--) {
    if(strcmp(symbol_table[i].name, name) == 0 
       && symbol_table[i].kind & kind)
...
void set_level(int index, unsigned level){
 if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    symbol_table[index].level = level;
}

unsigned get_level(int index) {
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
